...
theorem WOSet.le_or_lt (hx : x ∈ W.S) (hy : y ∈ W.S) : W.le x y ∨ W.lt y x := by
  let xy := {z | z = x ∨ z = y}
  have hxyS : xy ⊆ W.S := pair_subset hx hy
  have hxxy : x ∈ xy := by
    apply mem_setOf.mpr
    left
    rfl
  have hyxy : y ∈ xy := by
    ...
    right
    rfl
  have hxy : xy.Nonempty := nonempty_of_mem hxxy
  obtain ⟨z, hzxy⟩ := W.exists_min xy hxyS hxy
  obtain (hzx | hzy) := hzxy.1
  · rw [hzx] at hzxy
    left
    apply hzxy.2 y hyxy
  rw [hzy] at hzxy
...
theorem WOSet.eq_iff : W = W' ↔ W.S ⊆ W'.S ∧ ∀ x y, W'.le x y → W.le x y := by
  ...
  · constructor
    ...
    exact hss hx
    intro hx
    apply (h x x (W'.refl x hx)).mem_left
  constructor
  intro hxy
  · by_contra h2
    rw [← W'.lt_iff_not_le (mem_of_mem_of_subset hxy.mem_right hss) (mem_of_mem_of_subset hxy.mem_left hss)] at h2
    have h3 : x = y := by
      exact hxy.antisymm (h y x h2.le)
    rw [eq_comm] at h3
    have h4 : y ≠ x := by
      exact h2.ne
    exact h4 h3
...
theorem IsInitialSegment.le_iff_le (h : IsInitialSegment W W') (hy : y ∈ W.S) :
  ...
  constructor
  · exact h.le_of_le
  obtain ⟨z, hz⟩ := h.eq_setOf_lt
  intro hxy
  by_contra h2
  rw [← W.lt_iff_not_le hy] at h2
  have h3 : x = y := by
    exact hxy.antisymm (h.le_of_le h2.le)
  rw [eq_comm] at h3
  have h4 : y ≠ x := by
    exact h2.ne
  exact h4 h3
...
theorem IsInitialSegment.ssubset (h : IsInitialSegment W W') : W.S ⊂ W'.S := by
  ...
  constructor
  · exact hz.1
  rw [hz.2, nmem_setOf_iff, ← W'.le_iff_not_lt hz.1 hz.1]
  exact W'.refl z hz.1
...
theorem IsInitialSegment.trans {W₁ W₂ W₃ : WOSet α} (h : IsInitialSegment W₁ W₂)
  ...
  constructor
  · intro x y hxy1
    apply h'.le_of_le (h.le_of_le hxy1)
  obtain ⟨z, hz⟩ := h.eq_setOf_lt
  use z
  constructor
  · apply mem_of_mem_of_subset hz.1 h'.subset
  rw [hz.2]
  ext y
  simp
  exact h'.lt_iff_lt hz.1
...
instance (α : Type*) : PartialOrder (WOSet α) where
  ...
  le_antisymm := by
    ...
    rintro (rfl | h')
    · rfl
    have h1 : W.S ⊆ W'.S := h.subset
    have h2 : ¬W.S ⊆ W'.S := not_subset_of_ssubset h'.ssubset
...
theorem WOSet.le_alt (h : ∀ x y, W.le x y → W'.le x y)
  ...
  · intro hy
    simp
    rw [W'.lt_iff_not_le (hss hy) hxW']
    by_contra hyW
    exact hxW (h_seg x y hyW hy)
  intro hy
  by_contra hyW
  have hy' : W'.le x y := by
    apply hx
    exact hy.le.mem_left
    exact hyW
  rw [W'.le_iff_not_lt hy'.mem_left hy'.mem_right] at hy'
  exact hy' hy
...
theorem WOSet.lt_insertAbove (W : WOSet α) (a : α) (ha : a ∉ W.S) : W < W.insertAbove a ha := by
  ...
  · intro hy
    simp [hy]
    exact ne_of_mem_of_not_mem hy ha
  intro hy
  obtain (h1 | h2 | h3) := hy.1
  · exact h1.mem_left
  · exact False.elim (hy.2 h2)
  exact h3
...
theorem eq_univ_of_maximal (W : WOSet α) (hW : IsMaximal W) : W.S = univ := by
  ...
  obtain ⟨a, ha⟩ := h
  have hW' : ∃ (b : WOSet α), W ≤ b ∧ W ≠ b := by
    use W.insertAbove a ha
    rw [← lt_iff_le_and_ne]
    exact W.lt_insertAbove a ha
  obtain ⟨b, hb⟩ := hW'
  exact hb.2 (hW b hb.1)
end Insert
...
theorem IsChain.eq_or_segment_or_segment (hWs : IsChain Ws) (hW : W ∈ Ws) (hW' : W' ∈ Ws) :
  ...
  rw [le_iff_eq_or_lt, or_assoc] at h
  obtain (heq | hlt | hlt) := h
  · simp [heq]
  · simp [← W.lt_iff_initialSegment, hlt]
  simp [← W'.lt_iff_initialSegment, hlt]
...
def unionChain (Ws : Set (WOSet α)) (hWs : IsChain Ws) : WOSet α where
  ...
  supportedOn := by
    ...
    constructor
    · use W
      simp [hW, hxy.mem_left]
    use W
    simp [hW, hxy.mem_right]
  ...
  trans := by
    ...
    obtain (rfl | hseg | hseg) := h_cases
    · use W
      simp [hW, hxy.trans hyz]
    · use W'
      simp [hW', (hseg.le_of_le hxy).trans hyz]
    use W
    simp [hW, hxy.trans (hseg.le_of_le hyz)]
  ...
  exists_min := by
    ...
    have hW : ∃ W ∈ Ws, (W.S ∩ T).Nonempty := by
      ...
      have hxU : x ∈ ⋃ i ∈ Ws, i.S := hT hxT
      rw [mem_iUnion] at hxU
      obtain ⟨W, hxW⟩ := hxU
      simp at hxW
      have hxWT : x ∈ W.S ∩ T := mem_inter hxW.2 hxT
      have hWT : W.S ∩ T = ∅ := hcon W hxW.1
      have hWT' : ¬W.S ∩ T = ∅ := by
        exact ne_of_mem_of_not_mem' hxWT fun a ↦ a
      exact hWT' hWT
    obtain ⟨W, hW, hWT⟩ := hW

    -- use `h_min` to find a minimum `b` of `W.S ∩ T`.
    have h_min := W.exists_min (W.S ∩ T) inter_subset_left
    simp only [mem_inter_iff, and_imp] at h_min

    obtain ⟨b, ⟨hbW, hbT⟩, hbmin⟩ := h_min hWT
    ...
    by_cases htW : t ∈ W.S
    · use W
      simp [hW, hbmin t htW ht]

    -- Since `t ∈ W'.S \ W.S` but `W` and `W'` are in a chain,
    -- we know `W` is an initial segment of `W'`.
    have hseg : IsInitialSegment W W' := by
      obtain (rfl | hseg | hseg) := hWs.eq_or_segment_or_segment hW hW'
      · exact False.elim (htW htW')
      · exact hseg
      have hsub := hseg.subset
      exact False.elim (htW (hsub htW'))
    use W', hW'
    obtain ⟨c, hc⟩ := hseg.eq_setOf_lt
    by_cases hct : W'.le c t
    · have hxc : W'.lt b c := by
        rw [hc.2] at hbW
        exact hbW
      apply (hxc.trans_le hct).le
    rw [← W'.lt_iff_not_le htW' hc.1] at hct
    have : t ∈ W.S := by
      rw [hc.2]
      exact hct
    exact False.elim (htW this)
...
theorem unionChain_isUB (Ws : Set (WOSet α)) (hWs : IsChain Ws) :
  ...
  have h : ∃ W' ∈ Ws, IsInitialSegment W W' := by
    ...
    have hS : W.S = (unionChain Ws hWs).S := by
      ...
      intro W' hW'
      obtain (rfl | hseg | hseg) := hWs.eq_or_segment_or_segment hW hW'
      · exact subset_refl W.S
      · exact False.elim (h' W' hW' hseg)
      exact hseg.subset
    ext x y
    · simp
      constructor
  ...
  intro hy
  obtain ⟨W'', hW'', hW''y⟩ := hy
  exact hWs.lt_of_lt hW'' hW' hW''y hx
