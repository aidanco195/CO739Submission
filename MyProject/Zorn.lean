...
theorem IsChain.le_or_lt_of_mem_of_mem (hC : IsChain C) (hxC : x ∈ C) (hyC : y ∈ C) :
    x ≤ y ∨ y < x := by
  cases hC.le_or_le_of_mem_of_mem hxC hyC with
  | inl xley =>
    left
    exact xley
  | inr ylex =>
    cases eq_or_lt_of_le ylex with
    | inl yeqx =>
      left
      apply le_of_eq
      rw [eq_comm]
...
theorem IsSegmentOf.trans (h : IsSegmentOf S C) (h' : IsSegmentOf C D) : IsSegmentOf S D := by
  ...
  · exact h'.chain_right
  intro x y hxD hyS hxy
  exact h.mem_of_mem_of_le (h'.mem_of_mem_of_le hxD (mem_of_mem_of_subset hyS h.subset) hxy) hyS hxy
...
theorem IsSegmentOf.exists_strictUB_mem_of_ne (h : IsSegmentOf S U) (hne : S ≠ U) :
  ...
  have h_ssubset : S ⊂ U := by
    apply ssubset_of_ne_of_subset hne h.subset
  ...
  have le_or_lt : d ≤ c ∨ c < d := by
    exact h.chain_right.le_or_lt_of_mem_of_mem hdU (mem_of_mem_of_subset hcS h.subset)

  obtain (hdc | hcd) := le_or_lt
  · -- Use the fact that `S` is a segment of `U` to get a contradiction in this case.
    have hdS' : d ∈ S := by
      exact h.mem_of_mem_of_le hdU hcS hdc
    contradiction
  exact hcd
...
theorem GoodWRT_sUnion_chain (b : Set α → α) (hb : ∀ C, IsChain C → b C ∉ C) :
  ...
  · -- use the fact that `Sx` is a chain.
    apply hSx.1.le_or_le_of_mem_of_mem hxSx (mem_of_mem_of_subset hySy h.subset)
  -- use the fact that `Sy` is a chain.
  apply hSy.1.le_or_le_of_mem_of_mem (mem_of_mem_of_subset hxSx h.subset) hySy
...
theorem GoodWRT.insert_ub (b : Set α → α) (hb : ∀ C, IsChain C → IsStrictUB C (b C))
  ...
  have hbCS : b C ∉ S := by
    ...
    intro x hxC
    apply hSeg.mem_of_mem_of_le (mem_insert_of_mem (b C) hxC) hbCS ((hb C h.1).isUB x hxC)
  ...
  have hSC : IsSegmentOf S C := by
    refine ⟨hS,h.1,?_⟩
    intro x y hxC hyS hxy
    apply hSeg.mem_of_mem_of_le (mem_insert_of_mem (b C) hxC) hyS hxy
  ...
  · -- Since `b C ≤ y ∈ S ∪ {b s} ⊆ C`, this contradicts `b C` being a strict UB for `C`.
    have hxy' : ¬ b C ≤ y := by
      apply not_le_of_lt
      exact (hb C h.1) y (mem_of_mem_of_subset hy hSeg'.subset)
    contradiction
...
theorem zorn (h : ∀ C, IsChain C → ∃ (b : α), IsUB C b) : ∃ (m :α ), IsMaximal m := by
  ...
  have h_strictUB : ∀ (C : Set α), ∃ (b : α), IsChain C → IsStrictUB C b := by
    ...
    · -- use `h` and `h_con` to find a strict upper bound.
      obtain ⟨m, hmC⟩ := h C hC
      obtain ⟨b, hmb⟩ := h_con m
      rw [← lt_iff_le_and_ne] at hmb
      use b
      intro _ x hxC
      exact lt_of_le_of_lt (hmC x hxC) hmb
  ...
  have hb_not_mem : ∀ C, IsChain C → b C ∉ C := by
    ...
    intro C hC
    exact IsStrictUB.not_mem (hb C hC)
  ...
  have forall_good_segment : ∀ D, GoodWRT b D → IsSegmentOf D U := by
    ...
    intro D hbD
    refine ⟨?_,?_,?_⟩
    · exact subset_sUnion_of_subset {C | GoodWRT b C} D (fun ⦃a⦄ a => a) hbD
    · exact hU_chain
    intro x y hxU hyD hxy
    unfold GoodWRT at hbD
    obtain ⟨C, hbC, hxC⟩ := mem_sUnion.mp hxU
    obtain (hCD | hDC) := goodWRT_comparability hb_not_mem hbD hbC
    · exact mem_of_mem_of_subset hxC hCD.subset
    exact IsSegmentOf.mem_of_mem_of_le hDC hxC hyD hxy
  ...
  have hU_good : GoodWRT b U := by
    ...
    have hSD : S ⊆ D := by
      intro x hxS
      apply hDU.mem_of_mem_of_le (mem_of_mem_of_subset hxS hS.subset) hdD (hSd.isUB x hxS)

    have hSD_seg : IsSegmentOf S D :=
      hS.subset_right hD.1 hSD (hDU.subset)
...
-- The problem comes in h_strictUB, which asserts that every chain has a strict upper bound,
-- but if the assumption is removed, such a strict upper bound cannot be chosen.
